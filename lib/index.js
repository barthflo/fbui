...
const Button = ({ primary = true, backgroundColor, size = "medium", onClick, label, }) => {
    const mode = primary
        ? "storybook-button--primary"
        : "storybook-button--secondary";
    return (r.createElement(ButtonStyled, { type: "button", className: ["storybook-button", `storybook-button--${size}`, mode].join(" "), style: backgroundColor ? { backgroundColor } : {}, onClick: onClick }, label));
};
